...
export function useIntroLayout(count: number) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // -- Radial Positions (Initial State) --
      ...
      const radialRot = (angle * 180) / Math.PI + 90 + (rand2 - 0.5) * 16;
      
      const scale = isMobile ? 0.7 + rand3 * 0.3 : 0.85 + rand3 * 0.25;

      // -- Thread Positions (Final State) --
      ...
      // max sag in middle (x=0) is about 60px
      const normalizedX = threadX / (totalThreadWidth / 2); // -1 to 1
      const sag = (1 - normalizedX * normalizedX) * 60; // 0 at edges, 60 at center
