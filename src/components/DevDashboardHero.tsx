...
export function DevDashboardHero() {
  ...
  return (
    <section className="relative min-h-screen w-full flex flex-col justify-center items-center bg-black overflow-hidden px-6 pt-24 pb-12 border-b border-border/40">
      {/* Abstract Grid Overlay */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#0f172a_1px,transparent_1px),linear-gradient(to_bottom,#0f172a_1px,transparent_1px)] bg-[size:4rem_4rem] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_50%,#000_70%,transparent_100%)] opacity-35" />

      {/* Cybernetic Accent Glows */}
      <div className="absolute top-1/4 left-1/4 h-72 w-72 rounded-full bg-cyan-500/10 blur-[80px]" />
      <div className="absolute bottom-1/4 right-1/4 h-72 w-72 rounded-full bg-blue-600/10 blur-[80px]" />
      ...
      <div className="relative z-10 mx-auto max-w-[1200px] w-full flex flex-col lg:flex-row gap-12 items-center justify-between mt-8">
        ...
        <div className="flex flex-col items-start gap-6 max-w-xl text-left w-full">
          <div className="inline-flex items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-950/20 px-4 py-1.5 text-xs font-mono text-cyan-400 tracking-wider">
          ...
          <div className="grid grid-cols-2 gap-4 w-full mt-4 font-mono">
            <div className="bg-card/15 border border-border/40 p-4 rounded-xl flex items-center gap-3">
        ...
        <div className="w-full lg:max-w-xl bg-slate-950/95 border border-cyan-950/60 rounded-xl overflow-hidden shadow-2xl shadow-cyan-950/20 flex flex-col font-mono text-xs md:text-sm h-[320px]">
          {/* Terminal Window Header */}
          <div className="bg-slate-900 border-b border-cyan-950/40 px-4 py-2.5 flex items-center justify-between select-none">
          ...
          <div className="p-5 flex-1 flex flex-col gap-2 overflow-y-auto text-left scrollbar-none">
            ...
            <div className="h-px bg-cyan-950/35 my-1" />
            
            {logs.map((log, i) => {
              const isCommand = log.startsWith("systemctl") || log.includes(".sh");
              return (
                <div key={i} className="flex gap-2">
                  <span className="text-cyan-500 select-none">{isCommand ? "$" : ">"}</span>
                  <span className={isCommand ? "text-white font-medium" : "text-cyan-400/90"}>
                    {log}
                  </span>
                </div>
              );
            ...
            {logs.length < initialLogs.length && (
              <span className="h-4 w-1.5 bg-cyan-400 animate-pulse shrink-0 inline-block align-middle ml-1" />
            )}
          </div>
        </div>
      </div>
      ...
      {/* Bounce Down Arrow */}
      <div className="absolute bottom-8 flex flex-col items-center gap-1.5 text-gray-500 hover:text-cyan-400 transition-colors font-mono text-[10px] tracking-[0.2em] uppercase select-none">
