...
const getExtendedProject = (p: ReturnType<typeof useContent>["selectedWork"]["projects"][0]): TechProject => {
    ...
    <div className="min-h-screen bg-[#070b12] text-[#a8c4e0] font-mono selection:bg-[rgba(55,138,221,0.15)] selection:text-[#b8d4f0]">
      ...
      <section className="relative w-full h-screen flex flex-col justify-center items-center px-6 md:px-12">
        ...
        <div className="absolute bottom-8 left-6 md:left-12 text-[8px] uppercase tracking-[0.4em] text-[rgba(120,160,200,0.45)]">
          scroll<motion.span animate={{ opacity: [1, 0, 1] }} transition={{ duration: 1, repeat: Infinity }}>_</motion.span>
        </div>
            ...
            >
              ...
              <div className="relative z-10 flex justify-between items-end mt-8 text-[11px] uppercase tracking-widest text-[rgba(120,160,200,0.45)]">
                ...
                <span className="flex items-center gap-2">
                  ...
                  {p.status === "Archived" && <span className="w-1.5 h-1.5 rounded-full bg-[rgba(200,160,80,0.6)]" />}
                    ...
                    <pre className="text-[12px] font-mono leading-[1.8]">
                      <code>
                        {selectedProject.codeSnippet.split('\n').map((line, i) => {
                          ...
                          let hlLine = line;
                          if (line.includes('function ') || line.includes('return ') || line.includes('if ')) {
                            hlLine = line.replace(/(function|return|if)/g, '<span style="color:#6a9fd8">$1</span>');
                          }
                          if (line.includes('//')) {
                            hlLine = `<span style="color:rgba(120,160,200,0.3)">${line}</span>`;
                          }
                          if (line.includes("'")) {
                            hlLine = hlLine.replace(/'([^']+)'/g, '<span style="color:rgba(180,210,160,0.8)">\'$1\'</span>');
                          }
                          return (
                            <div key={i} dangerouslySetInnerHTML={{ __html: hlLine }} />
                          );
                        })}
                      </code>
                    </pre>
                ...
                <div className="flex flex-col gap-8 text-[9px] uppercase tracking-[0.2em] text-[rgba(120,160,200,0.45)]">
                  ...
                  {selectedProject.repo && (
                    <div>
                      ...
                      <a href={selectedProject.repo} target="_blank" rel="noreferrer" className="text-[#6a9fd8] hover:underline cursor-crosshair">
                        github.com/source
                      </a>
                    </div>
                  )}
